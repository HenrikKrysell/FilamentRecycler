...
class ClientHandler:
    def __init__(self, eventEmitter: EventEmitter):
        ...
        self.onRecievedMessageFromMicrocontroller = lambda *args: asyncio.create_task(
            self.__sendToClient(constants.RECIEVED_MESSAGE_FROM_MICROCONTROLLER, str(args[0])))
        self.onControllerStateMessage = lambda *args: asyncio.create_task(
            self.__sendToClient(constants.CONTROLLER_STATE_MESSAGE, args[0]))
        self._recieveBuffer = ''
    ...
    async def start(self, reader: asyncio.StreamReader, writer: asyncio.StreamWriter):
        ...
        self.reader = reader
        self.writer = writer
        self.eventEmitter.on(constants.RECIEVED_MESSAGE_FROM_MICROCONTROLLER,
                             self.onRecievedMessageFromMicrocontroller)
        self.eventEmitter.on(
            constants.CONTROLLER_STATE_MESSAGE, self.onControllerStateMessage)
    ...
    def unregisterEvent(self):
        self.eventEmitter.off(constants.RECIEVED_MESSAGE_FROM_MICROCONTROLLER,
                              self.onRecievedMessageFromMicrocontroller)
    ...
    async def __recieverThread(self):
        while self.isRunning:
            try:
                ...
                while True:
                    messageStart, messageEnd = self.__tryFindJSONMessage(
                        self._recieveBuffer)
                    if (messageStart >= 0 and messageEnd >= 0):
                        ...
                        if (message['topic'] == SetTargetRPMMessage.TOPIC):
                            self.eventEmitter.emit(
                                constants.FRONTEND_BASE_MESSAGE+message['topic'], SetTargetRPMMessage(message))
                        elif (message['topic'] == ShutdownMessage.TOPIC):
                            self.eventEmitter.emit(
                                constants.FRONTEND_BASE_MESSAGE+message['topic'], ShutdownMessage())
                        elif (message['topic'] == SetTargetTemperatureMessage.TOPIC):
                            self.eventEmitter.emit(
                                constants.FRONTEND_BASE_MESSAGE+message['topic'], SetTargetTemperatureMessage(message))
                        else:
                            self.eventEmitter.emit(
                                constants.FRONTEND_BASE_MESSAGE+message['topic'], message)
                    else:
                        break
            except Exception as e:
                print("FrontendComServer.Client: Exception {e}".format(e=e))
