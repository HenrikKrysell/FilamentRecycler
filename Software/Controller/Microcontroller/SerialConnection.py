    ...
    class Connection(asyncio.Protocol):
        def __init__(self, eventEmitter):
            ...
            self.eventEmitter.on(constants.SEND_MESSAGE_TO_MICROCONTROLLER,
                                 lambda *args: self.__sendMessage(args[0]))
        ...
        def consumeMessageBufferAndParseMessage(self):
            ...
            while invalidMessageType and not insufficentBytes and self.buffer.length > 0:
                parsedMessage, msgLength, invalidMessageType, insufficentBytes = parseMessage(
                    self.buffer)
        ...
        def data_received(self, data: bytes):
            try:
                ...
                while True:
                    ...
                    if parsedMessage:
                        print("SerialConnection::{msg}".format(
                            msg=parsedMessage))
                        self.eventEmitter.emit(
                            constants.RECIEVED_MESSAGE_FROM_MICROCONTROLLER, parsedMessage)
                    else:
                        break
